...
- **Web app:** a Flask application (API + simple web interface) serves the trained model, applying rule-based adjustments for weekends and holiday periods; SQLite stores the prediction history.
...
```
tfg-demand-forecasting/
├── README.md
├── requirements.txt
├── Notebook/                # model training & comparison (Google Colab)
│   ├── XGBOOST.ipynb
│   ├── RFR.ipynb
│   └── GradBoost2DIA.ipynb
└── webapp/                  # Flask API + web interface
    ├── app.py
    ├── models/
    │   └── XGBoostPedidosDeManana_Ajustado.joblib   # trained model
    └── templates/
        └── index.html
```

> The trained XGBoost model (`.joblib`) is included, so the web app runs out of the box. The local SQLite database is not included (it is created on first run).

## Tech stack

Python (pandas, NumPy, scikit-learn, XGBoost, Matplotlib, Joblib), Google Colab, Flask, SQLite.
